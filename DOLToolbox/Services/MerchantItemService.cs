    ...
    public class MerchantItemService
    {
        ...
        public async Task<string> Save(List<MerchantItem> models, string itemListId)
        {
            return await Task.Run(() =>
            {
                ...
                Util.ForEach(models.Where(x => !string.IsNullOrWhiteSpace(x.ItemListID)), x => x.ItemListID = itemListId);

                // remove deleted
                Util.ForEach(current.Where(x => !models.Select(s => s.ObjectId).Contains(x.ObjectId)), x => DatabaseManager.Database.DeleteObject(x));
